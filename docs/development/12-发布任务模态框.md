...
本章完成后，用户可以在任务大厅点击“发布任务”，填写任务标题、描述、预算、期望交付日期、招标截止时间、复杂度和付款账号，然后提交到 Laravel 后端。后端会校验表单，创建 `task`，保存付款账号快照，写入附件引用，并记录任务事件。

本章不创建独立的 `/tasks/create` 页面。发布任务属于任务大厅的高频操作，放在模态框中更符合当前 MVP 的交互。

> 最新说明：第 16 章已经将“任务描述”从 `Textarea` 升级为 Tiptap 富文本编辑器。第 18 章已经将“手工填写附件 ID”升级为“选择文件后立即上传”，并且发布任务提交 `attachments: [{ id, name }]`，最终写入 `attachment_ref.attachment_id` 和 `attachment_ref.attachment_name`。阅读当前最新代码时，任务描述请以 `RichTextEditor` / `RichTextViewer` 为准，附件请以 `TaskAttachmentUploader` 为准。
...
- 任务大厅右上角有“发布任务”按钮。
- 点击按钮后打开模态框。
- 模态框包含任务标题、任务描述、预算金额、复杂度、期望交付日期、招标截止时间、付款账号和附件上传。
- 付款账号来自后端调用外部付款账号列表接口后传给前端。
- 提交成功后关闭模态框，页面出现“任务已发布。”提示。
- 数据库写入 `task`、`task_event`，如果已经上传附件，还写入 `attachment_ref`。
...
- 附件由外部上传接口处理，TaskHub 只保存附件 ID 和名称。
...
| `resources/js/components/common/RichTextEditor.tsx` | 任务描述富文本编辑器，详见第 16 章 |
| `resources/js/components/common/RichTextViewer.tsx` | 任务描述富文本展示组件，详见第 16 章 |
| `resources/js/components/tasks/TaskAttachmentUploader.tsx` | 发布任务附件上传组件，详见第 18 章 |
...
# 当前项目使用一个轻量 Textarea、NativeSelect、付款账号 PaymentAccountCombobox，以及第 16 章新增的 RichTextEditor。
# 如果文件不存在，本章后面会给出完整代码。
```
...
return [
    ...
    // 查询全部付款账号的 path，用于发布任务弹窗中的可搜索账号选择器。
...
```env
# 外部交易/付款账号：发布任务时通过可搜索选择器选择账号，保存时写账号快照。
...
class PaymentAccountClient
{
     ...
     * 刷新付款账号缓存。
     *
     * 这个方法供已经拿到动态 SSO accessToken 的调用方刷新缓存；
     * 如果外部接口返回空列表或失败，不覆盖旧缓存。
     */
...
public function index(Request $request, PaymentAccountClient $paymentAccounts): Response
{
    ...
    return Inertia::render('Tasks/Index', [
        ...
        // 发布任务弹窗的付款账号搜索选择器数据。
        // 付款账号外部接口要求 Authorization Header，所以这里把当前登录人的 accessToken 传给 Client。
...
- `Dialog`：发布任务弹窗。
- `Textarea`：附件 ID 多行输入。
- `RichTextEditor`：任务描述富文本输入，详见第 16 章。
- `NativeSelect`：复杂度下拉框。
- `PaymentAccountCombobox`：付款账号可搜索选择器。
...
- 所以当前阶段保留一个项目内轻量封装 `NativeSelect` 处理固定枚举；付款账号数量较大，使用单独的 `PaymentAccountCombobox` 支持搜索。
            ...
            <form className="space-y-4" method="POST" onSubmit={submitCreateTask}>
                ...
                <Field label="付款账号" message={form.errors.paymentAccountId} required>
                    <PaymentAccountCombobox
                        // 付款账号来自外部接口，数据量可能比较大。
                        // 使用 Combobox 后可以按账号名称或账号 code 模糊搜索。
...
为什么这里不用 `<label>` 包住整个 `children`：

- 普通文本输入框可以通过 `label` 提升可访问性，但复合组件不能被一个大 `label` 整体包住。
- `DatePicker`、`DateTimePicker`、`PaymentAccountCombobox` 内部包含 Button、Popover 和多个 Input。
- 如果外层是 `<label>`，点击内部按钮或输入框时，浏览器会触发 label 的默认聚焦行为，容易导致 Popover 打开后马上关闭、输入框焦点异常、日期选不中。
- 后续如果某个普通 `Input` 需要明确 label 语义，应使用 `id + htmlFor` 单独关联，而不是把整个复合组件包进 label。
...
付款账号接口现在要求使用当前登录人的动态 SSO accessToken。只有浏览器完成 SSO 登录后，后端才能从 Session 中取到 token 并调用付款账号接口。后台命令没有浏览器 Session，当前不使用 `.env` 固定用户 token 访问外部接口。
...
- `Payment account options are unavailable.`：任务大厅捕获到了付款账号异常，页面会显示“付款账号列表不可用”。
- `Payment account request started.`：后端已经开始请求付款账号接口，可以看到 `base_url`、`path`、`method`、`verify_ssl`。
- `Payment account request connection failed.`：网络、域名、端口、证书或服务不可达。
- `Payment account request returned an unsuccessful status.`：外部接口返回了非 2xx 状态，日志里会有 `status` 和前 1000 个字符的 `body_preview`。
- `Payment account response is not JSON.`：接口响应不是 JSON，可能返回了 HTML 登录页、网关错误页或纯文本。
- `Payment account request succeeded.`：接口调用成功，日志里会显示 `data_count` 和 `total`。
...
- 确认浏览器当前 Session 中有有效 SSO accessToken。
- 打开 `storage/logs/laravel.log`，查看 `Payment account request succeeded.` 中的 `data_count` 是否大于 0。
