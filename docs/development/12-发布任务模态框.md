...
> 最新说明：第 16 章已经将“任务描述”从 `Textarea` 升级为 Tiptap 富文本编辑器。阅读当前最新代码时，任务描述请以 `RichTextEditor` / `RichTextViewer` 为准；`Textarea` 只继续用于附件 ID 等普通多行文本。
...
| `resources/js/components/ui/textarea.tsx` | Textarea 基础组件，当前主要用于附件 ID |
