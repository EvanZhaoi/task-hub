...
第 09 章已经完成“后端使用 SSO accessToken 查询当前登录人信息”，这个请求明确会带：

```text
Authorization: bearer {accessToken}
```

第 14 章的付款账号列表和本据点全员人员列表有一个额外问题：它们不只在用户页面请求中使用，也会被 `taskhub:refresh-external-directory-cache` 定时命令使用。

因此这里不能直接把“当前登录人的 accessToken”写死进两个大列表 Client，原因是：

- 页面请求中可以从当前 Session 读取登录人的 token。
- 定时命令运行时没有浏览器 Session，也没有“当前登录人”。
- 如果定时任务也必须调用外部接口，就需要总部或接口负责人确认一个稳定的授权方式，例如服务账号 token、客户端凭证、固定内网白名单或其它机制。

当前代码保持的原则：

- `SsoClient::fetchCurrentUser()` 已经按公司要求带 `Authorization: bearer {accessToken}`。
- `PaymentAccountClient` 和 `PersonnelClient` 先只封装 base URL、path、method、timeout、SSL 和 Redis 缓存。
- 如果付款账号和全员人员列表也要求 Authorization，下一步应先确认“定时刷新使用什么 token”，再统一修改两个 Client，不要在代码里临时硬编码某个人的 accessToken。
