...
- 定时刷新：命令由服务器定时执行，没有浏览器 Session，也没有当前登录人，因此使用 `.env` 中配置的服务端 token。
...
- `taskhub:sync-external-directories` 定时刷新时，使用 `PAYMENT_ACCOUNT_ACCESS_TOKEN`、`PERSONNEL_ACCESS_TOKEN` 或统一的 `EXTERNAL_DIRECTORY_ACCESS_TOKEN`。
...
# 外部大列表定时刷新使用的统一服务端 token。
# 如果付款账号和人员列表需要不同 token，可以分别配置下面两个专用字段。
EXTERNAL_DIRECTORY_ACCESS_TOKEN=
...
public function refreshCache(?string $accessToken = null): int
{
    ...
    // 定时任务没有浏览器 Session，所以这里使用配置中的服务端 token。
...
- 页面请求传当前登录人的 token；定时刷新使用 `.env` 中的服务端 token。
