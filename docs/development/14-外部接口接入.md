...
| `deptInfoList[0].obiCode` | `departmentId` | 部门编码，缺失时可用 `obiUuid` 兜底 |
| `deptInfoList[0].obiName` | `departmentName` | 部门名称，缺失时可用 `department` 兜底 |
| `eibEmail`、`eibPhoto` 等 | 暂不进入标准字段 | 当前 MVP 暂不使用，保留在 `raw` 中 |

注意：`deptInfoList` 是数组。当前 MVP 只需要一个部门用于展示和历史快照，所以取第一条有效部门。以后如果出现多部门权限判断，再单独扩展数据结构。
